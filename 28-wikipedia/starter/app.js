...
const fetchPages = async (searchValue) => {
    let searchValueCapitalized = searchValue.toUpperCase()
    resultsDOM.innerHTML = '<div class="loading"></div>';
    try {
        const response = await fetch (`${url}${searchValueCapitalized}`)
        const data = await response.json();
        const results = data.query.search;
        if(results.length < 1){
            resultsDOM.innerHTML = '<div class="error">no matching results. please try again later</div>';
            return
        }
